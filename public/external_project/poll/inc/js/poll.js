$(document).ready(function() {   
	...
	$("#pollSubmit").click(function() {
		...
		if ($('input:radio[name=pollAnswerID]:checked').length) {
			...
			$.ajax({  
				...
				success: function(theResponse) { 
					...
					if (theResponse == "voted") { 
						...
						$("pollMessage").show();
						$("#pollMessage").html("ลงประชามติเรียบร้อยแล้ว").fadeTo("slow", 1);
						$("#pollSubmit").attr("disabled", "disabled"); //disable the submit button
						$("#pollSubmit").attr("src", "images/vote_disabled.png");
					}
				}  
			});  
			return false; 
			...
			$("#pollMessage").html("กรุณาเลือกหัวข้อที่ต้องการ.").fadeTo("slow", 1, function(){
				setTimeout(function() {
