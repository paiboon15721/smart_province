...
function getListAA($cc){
    $sql = "select catm_ukey as tt_code, catm_desc from tabdb.ccaattmm where substr(catm_ukey,1,2) = {$cc} and (flag_area=2 or flag_area=5) and catm_tdate=0";
    try{
        $conn_id = connectORS();
        $stmt = prepare($conn_id,$sql);
        execute($stmt);
...
function getListTT($aa){
    $sql = "select catm_ukey as tt_code, catm_desc from tabdb.ccaattmm where substr(catm_ukey,1,4) = substr({$aa},1,4) and (flag_area=3 or flag_area=6)  and catm_tdate=0";
    try{
        $conn_id = connectORS();
        $stmt = prepare($conn_id,$sql);
        execute($stmt);
