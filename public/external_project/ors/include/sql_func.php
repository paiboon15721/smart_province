...
function getDataByPid($pid){
    //$sql = "select p.pid, (select title_print from tabdb.title where title_code=p.title), p.fname, p.lname, p.dob, sys.age(p.dob), (select nat_tdesc from tabdb.nat where nat_code = p.nat), p.own_st, p.pop_st, p.sex,  p.datemi, p.faname, (select nat_tdesc from tabdb.nat where nat_code =p.fnat), p.maname, (select nat_tdesc from tabdb.nat where nat_code =p.mnat), h.hid, h.hno, h.ccaattmm, nvl((select tnn_desc from tabdb.thanon where tnn_ukey= h.rcode || h.thanon),' '), nvl((select trok_desc from tabdb.trok where trok_ukey= h.rcode || h.trok),' '), nvl((select soi_desc1 || '|' || soi_desc2 from tabdb.soi where soi_ukey= h.rcode || h.soi),' | ') , (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,2) || '000000' and flag_area=1), (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,4) || '0000' and flag_area in (2,5)),(select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,6) || '00' and flag_area in (3,6)),(select catm_desc from tabdb.ccaattmm where catm_ukey=h.ccaattmm and flag_area=4) from ors.pop p join ors.house h on p.hid=h.hid where p.pid={$pid}";
    $sql = "select p.pid, (select title_print from tabdb.title where title_code=p.title), p.fname, p.lname, p.dob, sys.age(p.dob), (select nat_tdesc from tabdb.nat where nat_code = p.nat), p.own_st, p.pop_st, p.sex,  p.datemi, p.faname, (select nat_tdesc from tabdb.nat where nat_code =p.fnat), p.maname, (select nat_tdesc from tabdb.nat where nat_code =p.mnat), h.hid, h.hno, h.ccaattmm, nvl((select tnn_desc from tabdb.thanon where tnn_ukey= h.rcode || h.thanon),' '), nvl((select trok_desc from tabdb.trok where trok_ukey= h.rcode || h.trok),' '), nvl((select soi_desc1 || '|' || soi_desc2 from tabdb.soi where soi_ukey= h.rcode || h.soi),' | ') , (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,2) || '000000' and flag_area=1), (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,4) || '0000' and flag_area in (2,5)),(select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,6) || '00' and flag_area in (3,6)),(select catm_desc from tabdb.ccaattmm where catm_ukey=h.ccaattmm and flag_area=4),";
    $sql .= "case ";
...
function getHouseByCond($hid, $cc, $aa, $tt, $mm, $htype){
    $sql = "select h.hid,(select htype_desc from tabdb.htype where htype_code=h.htype), h.hid, h.hno, h.ccaattmm, nvl((select tnn_desc from tabdb.thanon where tnn_ukey= h.rcode || h.thanon),' '), nvl((select trok_desc from tabdb.trok where trok_ukey= h.rcode || h.trok),' '), nvl((select soi_desc1 || '|' || soi_desc2 from tabdb.soi where soi_ukey= h.rcode || h.soi),' | ') , (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,2) || '000000' and flag_area=1), (select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,4) || '0000' and flag_area in (2,5)),(select catm_desc from tabdb.ccaattmm where catm_ukey=substr(h.ccaattmm,1,6) || '00' and flag_area in (3,6)),(select catm_desc from tabdb.ccaattmm where catm_ukey=h.ccaattmm and flag_area=4) from ors.house h ";
...
function getListAA($cc){
    $sql = "select catm_ukey/10000 as tt_code, catm_desc from tabdb.ccaattmm where substr(catm_ukey,1,2) = {$cc} and flag_area in (2,5) and catm_tdate=0";
    try{
        $conn_id = connectORS();
        $stmt = prepare($conn_id,$sql);
        execute($stmt);
...
function getListMM($tt){
    $sql = "select catm_ukey as mm_code, catm_desc from tabdb.ccaattmm where substr(catm_ukey,1,6) = {$tt} and flag_area=4 and catm_tdate=0 order by catm_ukey";
    try{
        $conn_id = connectORS();
        $stmt = prepare($conn_id,$sql);
        execute($stmt);
