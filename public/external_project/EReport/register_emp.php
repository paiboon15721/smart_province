...
function Valid(form)
{
	...
	}else if(full_name == "ไม่พบชื่อนี้ ในจังหวัดนครนายก")
	{
...
if($sendpress == "insert")
{
	...
	{
		$work_place = $prov;
		//echo "work_place $work_place<br>"; exit();
	}else if(empty($tumbon)) {
		$work_place = $amper;
		//echo "work_place2 $work_place<br>"; exit();
	}else if(empty($mooban)) {
		$work_place = $tumbon;
		//echo "work_place3 $work_place<br>"; exit();
	}else{
		$work_place = $mooban;
		//echo "work_place4 $work_place<br>"; exit();
	}
	//================ INSERT ==============
	if($pid == "")
	{
		...
		$sql_insert = "insert into tab_e_regis values ('$pid' , '$fullname' , '$work_place' , '$p_level' , '$txt_post' , '$txt_add' , '$txt_tel' , '$txt_email' , '$upd_date' , '0' ,'$upd_date' , '$emp_id') ";
		...
		if(!($query))
		{
			...
			if (!empty($fileupload))
			{
				...
				$c = count ($array_last) - 1;
				$limitfile=1000000; //กำหนด ขนาด file ที่อนุญาติให้โหลดเข้ามาเก็บได้ (ต่อ 1 file) หน่วยเป็น byte
				$lastname = strtolower($array_last[$c]);
				if($fileupload_size < 50000)
				{
					if ($lastname == "gif" or $lastname == "jpg" or $lastname == "jpeg")
					{
						$photoname = $EMPID.".".$lastname;
						/*$file_dele_jpg = $EMPID.".jpg";
						$file_dele_jpeg = $EMPID.".jpeg";
						$file_dele_gif = $EMPID.".gif";
						unlink("./IMAGE_EMP/$file_dele_jpg");
						unlink("./IMAGE_EMP/$file_dele_jpeg");
						unlink("./IMAGE_EMP/$file_dele_gif");*/
						//copy($fileupload,"./IMAGE_EMP/".$photoname);
						move_uploaded_file($fileupload, "./IMAGE_EMP/".$photoname);
						chmod($photoname, 0755);
							//echo "photoname = $photoname"; exit();
						showmsg("upload รูปภาพเรียบร้อยแล้ว");
						
					}else{
						showmsg("ไม่สามารถ upload รูปภาพได้เนื่องจากไฟล์ไม่ถูกต้อง");
					}
					unlink($fileupload);
				}else{
					showmsg("ไม่สามารถ upload รูปภาพได้เนื่องจากไฟล์ใหญ่เกินไป");
				}
			}
...
<?php if(check_use($_SESSION['EMPID'])) {?>
...
<TR>
	<TD>
		...
		<tr height="30">
			...
			<td width="80%" align="left" valign="middle" id="fontred">**รูปภาพต้องเป็นไฟล์ jpg หรือ jpeg หรือ gif เท่านั้น และมีขนาดไม่เกิน 50 KB</td>
		</tr>
