...
<script type="text/javascript">
	...
	function changePage(page,selPage){
		$.get(page+"?ts=<?php echo time();?>", {selPage:selPage }, 
			function(data){$("#divGetData").html(data);$("#page_now").val(selPage);$('html, body').animate({scrollTop: $(document).height()}, 1500);}
		);}
	function check_info(){
...
if($systime >=800 && $systime < 1600){$phase=1;}
elseif($systime >=1600 && $systime < 2400){$phase=2;}
else{$phase=3;}
