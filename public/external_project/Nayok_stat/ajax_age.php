...
//----------------------------------------------------------------//		
	...
	?>
		...
		<div id="tabs2" style="margin:15px 0px 0px 0px; width:950px;">
				...
				<div id="tabs2-1">	<?php 	tab_age($yymm, $emp_job, $emp_rcode, $ccaattmm, 0);  ?>		</div> 
				<div id="tabs2-2">	<?php 	tab_age($yymm, $emp_job, $emp_rcode, $ccaattmm, 1);  ?>		</div>
				<div id="tabs2-3">	<?php	tab_age($yymm, $emp_job, $emp_rcode, $ccaattmm, 2);  ?>		</div>
				<div id="tabs2-4">	<?php	tab_age($yymm, $emp_job, $emp_rcode, $ccaattmm, 3);  ?>		</div>
		</div>		
	<?php
...
function tab_age($yymm, $emp_job, $emp_rcode, $ccaattmm, $nat_type){
	try{
			...
			switch ($nat_type){
				case 0 : $txt_label = "";						break;
				case 1 : $txt_label = "(สัญชาติไทย)";	break;
				case 2 : $txt_label = "(สัญชาติจีน)";		break;
				case 3 : $txt_label = "(อื่นๆ)";				break;
			}
		...
		if ($code == "1"){		
			...
			for($j=0;$j<21;$j++){
				...
				$male_range[$j] = $sum_male[4];
				$female_range[$j] = $sum_female[4];
				$all_range[$j] = $sum_summary[4];	
...
?>
		...
		<a id="<?php echo "multi".$nat_type; ?>" class="chart_multi" href="javascript:;"><img src="images/chart_multi.png" id="icon-multi" height="32" width="32"></a>
...
///// multi column ///////
					...
					// $multi_val = "";
					// for ($a=(-4);$a<=17;$a++){
						// $multi_val .= "[".sum_array2($arr_male,$init,$init+4).",";
						// $multi_val .= sum_array2($arr_female,$init,$init+4)."]";
						// $multi_val .= ($a != 17 ? "," : "");
						// $init +=  5 ;
					...
					$init = -4;
					for ($a=(-4);$a<=17;$a++){
						$json_male[$a+4] = sum_array2($arr_male,$init,$init+4);
						$json_female[$a+4] = sum_array2($arr_female,$init,$init+4);
						$init +=  5 ;
					}
					$multi_group = Array('อายุ <1','อายุ 1-5','อายุ 6-10','อายุ 11-15','อายุ 16-20','อายุ 21-25','อายุ 26-30','อายุ 31-35','อายุ 36-40','อายุ 41-45','อายุ 46-50','อายุ 51-55','อายุ 56-60','อายุ 61-65','อายุ 66-70','อายุ 71-75','อายุ 76-80','อายุ 81-85','อายุ 86-90','อายุ 90-95','อายุ 96-100','อายุ >100');
					$multi_cat 	= Array("ชาย","หญิง");
					$send_multi = Array("json_male" => $json_male ,
												"json_female" => $json_female ,
												"json_cat" => $multi_cat ,
												"json_group" => $multi_group);	
					// $send_multi = Array($json_male);	
...
<script type = "text/javascript">
					$(document).ready(function() {
							...
							var json_multi_val = '<?php echo  json_encode($send_multi);?>';
							
							var obj = jQuery.parseJSON('<?php echo  json_encode($send_multi);?>');				
							// alert(obj.json_male);
							
								$("#pie"+sub_page).click(function() {									
										$.fancybox.open({
											href : "include/chart_pie.php?value="+pie_val+"&page=ajax_age&sub_page="+sub_page+"&ccaattmm="+ccaattmm+"",
											type : 'iframe',
											padding : 5
										});
								});
								...
								$("#bar"+sub_page).click(function() {
										$.fancybox.open({
											href : "include/chart_bar.php?value="+bar_val+"&cat="+bar_cat+"&page=ajax_age&sub_page="+sub_page+"&ccaattmm="+ccaattmm+"",
											type : 'iframe',
											padding : 5
										});
								});
								...
								$("#multi"+sub_page).click(function() {
										$.fancybox.open({
											href : "include/chart_multi.php?obj="+json_multi_val+"&value=0&page=ajax_age&sub_page="+sub_page+"&ccaattmm="+ccaattmm+"",
											type : 'iframe',
											padding : 5
										});
								});
