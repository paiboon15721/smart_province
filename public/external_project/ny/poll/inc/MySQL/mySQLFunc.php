...
function getPidExistVote($pid){
	mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
	$query  = "SELECT pollID FROM polls WHERE polls.pollStatus = 1";
	$result = mysql_query($query);
	while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
		$pollID = $row['pollID'];	
	}
	
	$query  = sprintf("SELECT count(*) AS totalRow FROM pollTrans WHERE pollID = %d and pid = %s", $pollID, $pid);
	$result = mysql_query($query);
	while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
...
function addVote($pid, $answerID, $pollID){
	mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
	$query  = "UPDATE pollanswers SET pollAnswerPoints = pollAnswerPoints + 1 WHERE pollAnswerID = ".$answerID." and pollID = ".$pollID."";
	mysql_query($query);
	
	if(mysql_affected_rows()){ //-- database has been changed 
		$sql = sprintf("INSERT INTO `polls`.`pollTrans` (`pid`, `pollID`, `answer`) VALUES (%s,%d,%d)", $pid, $pollID, $answerID);
		mysql_query($sql);
		if(mysql_affected_rows()){ //-- database has been changed 
...
function getVoteName($id){
	mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
	$query  = "SELECT pollQuestion FROM polls WHERE pollID=".$id;
	$result = mysql_query($query);
...
function getVoteTotal(){
	mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
	$query  = "select sum(pollAnswerPoints) AS sumScore from pollanswers where pollId = (select pollId from polls where pollStatus = 1)";
	$result = mysql_query($query);
	while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
...
?>
            ...
            <tr>
              <td height="50" align="center" valign="middle"><span id="pollMessage"></span><img src="ajaxLoader.gif" alt="Ajax Loader" id="pollAjaxLoader" /></td>
            </tr>
      </table>
<?php
}
...
<?php
	function checkPollActive(){
		$strSQLSelect = "SELECT count(*) AS cntPoll FROM polls WHERE pollStatus = 1";
...
<?php
	function rollBack($pollID){
		mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
		$strSQL = "DELETE FROM polls WHERE `pollID` =".$pollID;
		$objQuery = mysql_query($strSQL);
		$strSQL = "DELETE FROM pollanswers  WHERE `pollID` =".$pollID;
		$objQuery = mysql_query($strSQL);
	}
	
	function insertPoll($txtPollTitle){
		
		mysql_query("SET NAMES 'utf8' COLLATE 'utf8_general_ci';");
		/* UPDATE pollStatus IN poll To Disable */
		$txtPollTitle = mysql_real_escape_string($txtPollTitle);
		$strSQL = "UPDATE polls SET pollStatus = 0";
		$objQuery = mysql_query($strSQL);
		
		/* SELECT LAST ROW IN poll */
		$strSQLSelect = "SELECT `pollID` FROM polls ORDER BY `pollID` DESC LIMIT 1";
		$objQuery = mysql_query($strSQLSelect);
		$row = mysql_fetch_array($objQuery);
		$pollID = $row['pollID'] + 1;
		...
		$strSQLInsert = "INSERT INTO polls ";
		$strSQLInsert .="(pollID, pollQuestion,pollStatus) ";
		$strSQLInsert .="VALUES ";
		$strSQLInsert .="(".$pollID.",'".$txtPollTitle."','1')";
		...
		$strSQLInsert = "INSERT INTO pollanswers ";
		$strSQLInsert .="(pollAnswerID,pollID, pollAnswerValue, pollAnswerPoints, pollAnswerListing) ";
		$strSQLInsert .="VALUES ";
		$strSQLInsert .="(1,".$pollID.",'เห็นด้วย', 0, 1)";
		$objQuery = mysql_query($strSQLInsert);
		...
		$strSQLInsert .="(pollAnswerID,pollID, pollAnswerValue, pollAnswerPoints, pollAnswerListing) ";
		$strSQLInsert .="VALUES ";
		$strSQLInsert .="(2,".$pollID.",'ไม่เห็นด้วย', 0, 2)";
		$objQuery = mysql_query($strSQLInsert);
